...
<body>
			...
			<nav>
				<ul class="pagination pg-blue justify-content-center">
					...
					<c:choose>
						...
						<c:otherwise>
							<li class="page-item"><a class="page-link" href="list.nc?page=1">First</a></li>
						</c:otherwise>
					</c:choose>
					<!-- 이전 -->
					<c:choose>
						<c:when test="${(page.curPage -1) < 1}">
							<li class="page-item disabled"><a class="page-link disabled" href="list.nc?page=${page.curPage -1}">Previous</a></li>
						</c:when>
						<c:otherwise>
							<li class="page-item"><a class="page-link" href="list.nc?page=${page.curPage -1}" >Previous</a></li>			
						</c:otherwise>
					</c:choose>
					...
					<c:otherwise>
						<li class="page-item"><a class="page-link" href="list.nc?page=${fEach}">${fEach}</a></li> &nbsp;						
					</c:otherwise>
					</c:choose>
					</c:forEach>
					...
					<c:choose>
						<c:when test='${(page.curPage + 1) > page.totalPage}'>
							<li class="page-item disabled"><a class="page-link disabled" href="list.nc?page=${page.curPage -1}">Next</a></li>
						</c:when>
						<c:otherwise>	
							<li class="page-item"><a class="page-link" href="list.nc?page=${page.curPage + 1}">Next</a></li>
						</c:otherwise>
					</c:choose>
					...
					<c:choose>
						...
						<c:otherwise>
							<li class="page-item"><a class="page-link" href="list.nc?page=${page.totalPage}" onclick = "prev();">Last</a></li>
						</c:otherwise>
					</c:choose>
				</ul>				
			</nav>
	...
	<div class = "container align-items-center">
		...
		<% if(session.getAttribute("admin") != null){ %>
      		...
      		<div align = "right" >
				<button type="button" class = "btn btn-primary" onclick ="location.href='write_view.nc'">글쓰기</button>
			</div>
		<% } %>	
	</div>
