			...
			<nav>
				<ul class="pagination pg-blue justify-content-center">
					...
					<c:choose>
						...
						<c:otherwise>
							<li class="page-item"><a class="page-link" href="search.bbs?page=1&sOption=<%= sOption %>&search=<%= search %>">First</a></li>
						</c:otherwise>
					</c:choose>
					<!-- 이전 -->
					<c:choose>
						<c:when test="${(page.curPage -1) < 1}">
							<li class="page-item disabled"><a class="page-link disabled" href="search.bbs?page=${page.curPage -1}&sOption=<%= sOption %>&search=<%= search %>">Previous</a></li>
						</c:when>
						<c:otherwise>
							<li class="page-item"><a class="page-link" href="search.bbs?page=${page.curPage -1}&sOption=<%= sOption %>&search=<%= search %>" >Previous</a></li>			
						</c:otherwise>
					</c:choose>
					...
					<c:otherwise>
						<li class="page-item"><a class="page-link" href="search.bbs?page=${fEach}&sOption=<%= sOption %>&search=<%= search %>">${fEach}</a></li> &nbsp;						
					</c:otherwise>
					</c:choose>
					</c:forEach>
					...
					<c:choose>
						<c:when test='${(page.curPage + 1) > page.totalPage}'>
							<li class="page-item disabled"><a class="page-link disabled" href="search.bbs?page=${page.curPage -1}&sOption=<%= sOption %>&search=<%= search %>">Next</a></li>
						</c:when>
						<c:otherwise>	
							<li class="page-item"><a class="page-link" href="search.bbs?page=${page.curPage + 1}&sOption=<%= sOption %>&search=<%= search %>">Next</a></li>
						</c:otherwise>
					</c:choose>
					...
					<c:choose>
						...
						<c:otherwise>
							<li class="page-item"><a class="page-link" href="search.bbs?page=${page.totalPage}&sOption=<%= sOption %>&search=<%= search %>" onclick = "prev();">Last</a></li>
						</c:otherwise>
					</c:choose>
				</ul>				
			</nav>
