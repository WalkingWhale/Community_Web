	...
	<nav class="navbar navbar-expand-lg navbar-light bg-light">
  		...
  		<div class="collapse navbar-collapse" id="navbarTogglerDemo02">
    		<ul class="navbar-nav mr-auto mt-2 mt-lg-0">
				...
				<% } %> 
				<li>      			
        			<a class="nav-link" href="../page_way/way.jsp">찾아오시는 길</a>
      			</li>
      		</ul>
			...
			<nav>
				<ul class="pagination pg-blue justify-content-center">
					...
					<c:choose>
						...
						<c:otherwise>
							<li class="page-item"><a class="page-link" href="search.user?page=1&sOption=<%= sOption %>&search=<%= search %>">First</a></li>
						</c:otherwise>
					</c:choose>
					<!-- 이전 -->
					<c:choose>
						<c:when test="${(page.curPage -1) < 1}">
							<li class="page-item disabled"><a class="page-link disabled" href="search.user?page=${page.curPage -1}&sOption=<%= sOption %>&search=<%= search %>">Previous</a></li>
						</c:when>
						<c:otherwise>
							<li class="page-item"><a class="page-link" href="search.user?page=${page.curPage -1}&sOption=<%= sOption %>&search=<%= search %>" >Previous</a></li>			
						</c:otherwise>
					</c:choose>
					...
					<c:otherwise>
						<li class="page-item"><a class="page-link" href="search.user?page=${fEach}&sOption=<%= sOption %>&search=<%= search %>">${fEach}</a></li> &nbsp;						
					</c:otherwise>
					</c:choose>
					</c:forEach>
					...
					<c:choose>
						<c:when test='${(page.curPage + 1) > page.totalPage}'>
							<li class="page-item disabled"><a class="page-link disabled" href="search.user?page=${page.curPage +1}">Next</a></li>
						</c:when>
						<c:otherwise>	
							<li class="page-item"><a class="page-link" href="search.user?page=${page.curPage+1}&sOption=<%= sOption %>&search=<%= search %>">Next</a></li>
						</c:otherwise>
					</c:choose>
					...
					<c:choose>
						...
						<c:otherwise>
							<li class="page-item"><a class="page-link" href="search.user?page=${page.totalPage}&sOption=<%= sOption %>&search=<%= search %>">Last</a></li>
						</c:otherwise>
					</c:choose>
				</ul>				
			</nav>
