		...
		try {			
			...
			if(pOption.equals("1day")) {
				query = "select * " +
					   	   ...
					   	   "			from ( " +
					   	   "				select * " +
					   	   "				from mvc_board where btype = ? and bDate >= sysdate-2"+
					       "			order by bGroup desc, bStep asc ) A "+
					   	   "	where rownum <= ? ) B " +
					       "where B.num >= ?";
			} else if(pOption.equals("1weak")) {
				query = "select * " +
					   	   "	from ( "+
					   	   "		select rownum num, A.* " +
					   	   "			from ( " +
					   	   "				select * " +
					   	   "				from mvc_board where btype = ? and bDate > sysdate-8"+
					       "			order by bGroup desc, bStep asc ) A "+
					   	   "	where rownum <= ? ) B " +
					       "where B.num >= ?";
				...
				query = "select * " +
					   	   ...
					   	   "			from ( " +
					   	   "				select * " +
					   	   "				from mvc_board where btype = ? and bDate > sysdate-32"+
					       "			order by bGroup desc, bStep asc ) A "+
					   	   "	where rownum <= ? ) B " +
					       "where B.num >= ?";
				...
				query = "select * " +
					   	   ...
					   	   "			from ( " +
					   	   "				select * " +
					   	   "				from mvc_board where btype = ? and bDate > sysdate-94"+
					       "			order by bGroup desc, bStep asc ) A "+
					   	   "	where rownum <= ? ) B " +
					       "where B.num >= ?";
				...
				query = "select * " +
					   	   ...
					   	   "			from ( " +
					   	   "				select * " +
					   	   "				from mvc_board where btype = ? and bDate > sysdate-187"+
					       "			order by bGroup desc, bStep asc ) A "+
					   	   "	where rownum <= ? ) B " +
					       "where B.num >= ?";
			}
